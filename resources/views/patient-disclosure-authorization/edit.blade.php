...
@section('page_title')
Edit: Patient Disclosure Authorization Form
@endsection
	...
	<div class="col-sm-8 col-xs-12">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<form class="validateForm" id="patient_disclosure_authorization_form" method="POST" action="{{ route('patient.disclosure.update', $form->uuid) }}" role="form">

					@csrf

					<p>I, <input type="text" name="patient_name" value="{{ $form->patient_name }}" class="validate[required, maxSize[255]]" /> hereby authorize the use or disclosure of my protected health information as described below:</p>

					<h4>1. <u>AUTHORIZED PERSONS TO USE AND DISCLOSE PROTECTED HEALTH INFORMATION</u></h4>

					<input type="text" name="section_one_data[field1]" class="validate[required, maxSize[255]]" value="{{ $form->section_one_data['field1'] }}" /> is authorized to disclose the following protected health information to <input type="text" name="section_one_data[field2]" class="validate[required, maxSize[255]]" value="{{ $form->section_one_data['field2'] }}" /> of <input type="text" name="section_one_data[field3]" class="validate[required, maxSize[255]]" value="{{ $form->section_one_data['field3'] }}" />, <input type="text" name="section_one_data[field4]" class="validate[required, maxSize[255]]" value="{{ $form->section_one_data['field4'] }}" />.
					...
					<p>The purpose of this use or disclosure is <input type="text" name="form_purpose" class="validate[required, maxSize[255]]" value="{{ $form->form_purpose }}" />.</p>

					<h4>4. <u>VALIDITY OF AUTHORIZATION FORM</u></h4>
					<p>This Authorization Form is valid beginning on <input type="text" class="datepicker validate[required, custom[date]]" placeholder="{{ config('app.DATE_FORMAT_PLACEHOLDER') }}" name="authorization_start" autocomplete="off" value="{{ $form->authorization_start }}" /> and expires on <input type="text" class="datepicker validate[required, custom[date]]" placeholder="{{ config('app.DATE_FORMAT_PLACEHOLDER') }}" name="authorization_expiry" autocomplete="off" value="{{ $form->authorization_expiry }}" />.</p>
					...
					<div class="row">
						<div class="col-sm-6 col-xs-12">
							<strong>By:</strong> <input type="text" name="acknowledgement_by" class="validate[required, maxSize[255]]" value="{{ $form->acknowledgement_by }}" />
						</div>

						<div class="col-sm-6 col-xs-12">
							<strong>Date:</strong> <input type="text" value="{{ \Carbon\Carbon::now()->format( config('app.VIEW_DATE_FORMAT') ) }}" placeholder="{{ config('app.DATE_FORMAT_PLACEHOLDER') }}" />
						</div>
					</div>
					...
					<div class="row">
						<div class="form-group">
							...
							<input type="hidden" name="signature_str_svg" id="patient_signature_svg" value="{{ $form->signature_str_svg }}" />
							<input type="hidden" name="signature_str_base" id="patient_signature_base" value="{{ $form->signature_str_base }}" />
						</div>
					</div>
