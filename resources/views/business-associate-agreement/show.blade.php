	...
	<div class="col-sm-8 col-xs-12">
		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				...
				<form id="business_associate_agreement_form" method="POST" action="{{ route('business-associate-agreement.update', $form->uuid) }}" role="form">
					
					@csrf

					<p>This Business Associate Agreement is made a part of the contract ("Contract") by and between <u>{{ $form->covered_entity }}</u> the Covered Entity ("CE") and <u>{{ $form->business_associate }}</u> the Business Associate ("BA"), Dated <u>{{ \Carbon\Carbon::parse( $form->agreement_dated_on )->format( config('app.VIEW_DATE_FORMAT') ) }}</u>.</p>

					<p>This Agreement is effective as of </u>{{ \Carbon\Carbon::parse( $form->effective_date )->format( config('app.VIEW_DATE_FORMAT') ) }}</u> (the "Agreement Effective Date").</p>
					...
					<div class="row">
						...
						<div class="col-sm-12 col-xs-12">
							...
							<p><u>{{ $form->covered_entity_name_footer }}</u></p>
							
							<div class="form-group">
								<label class="col-sm-1 col-xs-12">By:</label>
								<img src="data:{{ $form->covered_entity_signature_svg }}" />
							</div>

							<div class="clearfix"></div>
							<p>&nbsp;</p>
							<p><strong>Print Name:</strong> <u>{{ $form->coverted_entity_print_name }}</u></p>
							<p><strong>Title:</strong> <u>{{ $form->covered_entity_title }}</u></p>
							<p><strong>Date:</strong> <u>{{ \Carbon\Carbon::parse( $form->covered_entity_date )->format( config('app.VIEW_DATE_FORMAT') ) }}</u></p>
						</div>

						<div class="col-sm-12 col-xs-12">
							<h4>BUSINESS ASSOCIATE</h4>

							<p><u>{{ $form->business_associate_name_footer }}</u></p>
							
							<div class="form-group">
								<label class="col-sm-1 col-xs-12">By:</label>
								<img src="data:{{ $form->business_associate_signature_svg }}" />
							</div>

							<div class="clearfix"></div>
							<p>&nbsp;</p>

							<p><strong>Print Name:</strong> <u>{{ $form->business_associate_print_name }}</u></p>
							<p><strong>Title:</strong> <u>{{ $form->business_associate_title }}</u></p>
							<p><strong>Date:</strong> <u>{{ \Carbon\Carbon::parse( $form->business_associate_date )->format( config('app.VIEW_DATE_FORMAT') ) }}</u></p>
						</div>
					</div>
